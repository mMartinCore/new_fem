...
{ 
    ...
    {
        UserADmin::destroy($this->modelId);
    ...
    { 
        UserADmin::whereIn('id', $this->selected)->delete(); 
     ...
     { 
        UserADmin::whereIn('id', $this->selected); 
     }


    public function render()
    {
        $data = $this->search ==''? UserADmin::orderBy($this->sortField, $this->sortAsc ? 'asc' : 'desc')
        ->paginate($this->perPage) 
        : UserADmin::where('name', 'LIKE', "%{$this->search}%")
       ...
       // ->orWhere( 'var21', 'LIKE', '%'. $this->search .'%')
        ...
        return view('livewire.user-a-dmins.index', [
            'data' =>  $data ,
        ]);
