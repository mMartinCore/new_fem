...
<style>
  ...
  th:not(:last-child) {
    border-bottom: 2px solid rgba(0, 0, 0, .1);
     ...
     <div class="  mt-6  mx-auto px-4 sm:px-4">
        <a class="text-lg leading-6 font-medium text-gray-900 px-8">
            {{ $user }}
        </a>
    </div>
            ...
            <div class="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
                <div class="inline-block min-w-full shadow-lg rounded-lg overflow-hidden">
                      ...
                      <tr class=" flex flex-col flex-no wrap sm:table-row rounded-l-lg sm:rounded-none mb-2 sm:mb-0">       
                        ...
                        @include('include.table.table_th',  ['field' => 'name', 'label' => 'Merge Package Name'])
                        @include('include.table.table_th',  ['field' => 'packagestatus_id', 'label' => 'status'])
                        {{--
             ...
             <tbody class="flex-1 sm:flex-none">
                    @forelse($data as $row)
                         <tr class="flex flex-col  flex-no wrap sm:table-row mb-2 sm:mb-0">
                             ...
                             @include('include.table.tableAction',  ['table_name' => 'merges','model_name'=> $row]) 
                                    {{-- <td class="px-6 py-2 flex justify-end">

                                    <a class='inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs 
                                text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring focus:ring-gray-300 disabled:opacity-25 transition'
                                        href="{{ route('merges.show',$row->id) }}">
                                        {{ __('View') }}
                                    </a>
                                ...
                                text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring 
                                focus:ring-gray-300 disabled:opacity-25 transition'
                                        href="{{ route('merges.edit', $row->id) }}">
                                        {{ __('Edit') }}
                                    </a>
