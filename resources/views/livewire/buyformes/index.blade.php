...
<div class="flex items-center justify-end px-4 py-3 text-right sm:px-6">
    <a   class="text-lg leading-6 font-medium text-gray-900 mr-12 pr-8">
       {{$user}}
        ...
        <x-create-btn-link url="buyformes.create" >
        {{ __('Add BUY FOR ME PRODUCT') }}
      </x-create-btn-link>
    </div>
            ...
            <div class="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
                <div class="inline-block min-w-full shadow-lg rounded-lg overflow-hidden">
                      ...
                      <tr class=" flex flex-col flex-no wrap sm:table-row rounded-l-lg sm:rounded-none mb-2 sm:mb-0">       
                        ...
                        @include('include.table.table_th',  ['field' => 'piece', 'label' => 'piece'])
                        @include('include.table.table_th',  ['field' => 'link', 'label' => 'link'])                 
                        @include('include.table.table_th',  ['field' => 'reference', 'label' => 'reference'])
             ...
             <tbody class="flex-1 sm:flex-none">
                    @forelse($data as $row)
                         <tr class="flex flex-col  flex-no wrap sm:table-row mb-2 sm:mb-0">
                              ...
                              @include('include.table.table_td',   ['data' => $row->piece])  
                              @include('include.table.table_td',  ['data' => $row->link])                               
                              @include('include.table.table_td',  ['data' => $row->reference])  
                              @include('include.table.table_td',  ['data' => $row->description])
                              @include('include.table.table_td',  ['data' => $row->status])     
                              ...
                              @if ($row->status == 'Unpaid')
                                <a  class ='inline-flex items-center  ml-2 px-4 py-2 bg-site_color_theme border border-transparent rounded-md font-semibold text-xs 
                                   ...
                                   focus:border-gray-900 focus:ring focus:ring-gray-300 disabled:opacity-25 transition'href="{{ route('buyformes.payment-intent', $row->id )}} ">
                                     {{ __('Make Payment') }}
                              </a> 
                              @else                       
                                  <a  class ='disabled inline-flex items-center  ml-2 px-8 py-2 bg-site_color_theme border border-transparent rounded-md font-semibold text-xs 
                                   text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none
                                   focus:border-gray-900 focus:ring focus:ring-gray-300 disabled:opacity-50 transition'href="{{ route('buyformeInvoice',$row->id)}}">
                                     {{ __('Complete') }}
                              </a> 
                              @endif 
                              </td>

                             {{-- @include('include.table.tableAction',  ['table_name' => 'buyformes','model_name'=> $row])  --}}
                            </td>
                        </tr>
                        @empty
                        <tr>
