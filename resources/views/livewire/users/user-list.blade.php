...
{{-- <div class="flex items-center justify-end px-4 py-3 text-right sm:px-6">
        <x-create-btn-link url="users.create" >
        {{ __('Create') }}
      </x-create-btn-link>
    </div> --}}
    <div class="p-4 mt-8   mx-auto px-4 sm:px-4 shadow-lg rounded-lg border-2">      
         ...
         <div class="col-span-6  sm:col-span-3"> 
                 <select  wire:model="countryId" class="block appearance-none rounded-md w-full border border-1 border-gray-500 text-gray-700 py-2
                                               px-2 pr-8 round leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                    <option value="">Select Country</option>
                    @foreach ($countries as $country)
                    <option value="{{ $country->id }}">
                      {{-- <div class="flex items-center">
                            <div  style="width:1% !important; height:1% !important;" class="  flex-shrink-0 mr-2 sm:mr-3">
                              {!! $country->icon!!} 
                            </div> 
                        </div> --}}
                    {{ $country->name }}</option>                        
            ...
            <div class="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
                <div class="inline-block min-w-full shadow-lg rounded-lg overflow-hidden">
                      ...
                      <tr class=" flex flex-col flex-no wrap sm:table-row rounded-l-lg sm:rounded-none mb-2 sm:mb-0">       
                        ...
                        @include('include.table.table_th',  ['field' => 'Email', 'label' => 'Email'])
                        @include('include.table.table_th',  ['field' => 'country_id', 'label' => 'country'])
                        @include('include.table.table_th_no_sort',  [ 'label' => 'role'])
                        {{--
             
                        @include('include.table.table_th',  ['field' => 'address', 'label' => 'address'])
                        @include('include.table.table_th',  ['field' => 'city', 'label' => 'city'])
                        @include('include.table.table_th',  ['field' => 'phone', 'label' => 'phone'])
                    ...
                    @empty
                      <tr class="  flex flex-col flex-no wrap sm:table-row rounded-l-lg sm:rounded-none mb-2 sm:mb-0">       
                        ...
                        @include('include.table.table_th',  ['field' => 'city', 'label' => 'city'])
                        @include('include.table.table_th',  ['field' => 'phone', 'label' => 'phone'])
                        @include('include.table.table_th',  ['field' => 'role', 'label' => 'role'])
                        @include('include.table.table_th',  ['field' => 'team_id', 'label' => 'team_id'])
                        @include('include.table.table_th',  ['field' => 'var9', 'label' => 'var9'])
                        @include('include.table.table_th',  ['field' => 'var10', 'label' => 'var10'])
                        @include('include.table.table_th',  ['field' => 'var11', 'label' => 'var11'])
             ...
             <tbody class="flex-1 sm:flex-none">
                    @forelse($data as $row)
                         <tr class="flex flex-col  flex-no wrap sm:table-row mb-2 sm:mb-0">
                              ...
                              @include('include.table.table_td',  ['data' => $row->email]) 


                              
                                @include('include.table.table_td',  ['data' => $row->country->name]) 
                                @include('include.table.table_td',  ['data' => $row->roles->pluck('name')->implode(', ')])

                              {{-- @include('include.table.table_td',  ['data' => $row->country_id])  
                              @include('include.table.table_td',  ['data' => $row->address])  
                              @include('include.table.table_td',  ['data' => $row->city]) 

                              @include('include.table.table_td',  ['data' => $row->phone])  
                              @include('include.table.table_td',  ['data' => $row->role])  
                             ...
                             @include('include.table.tableAction',  ['table_name' => 'users','model_name'=> $row]) 
                            </td>
                        </tr>
                        @empty
                        <tr>
