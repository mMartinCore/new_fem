<x-jet-form-section submit="update">
       ...
       <div class="col-span-6 mt-4 sm:col-span-3">
        ...
        <div class="col-span-6 mt-4 sm:col-span-3">
          <x-jet-label for="role" value="{{ __('Role') }}" />
                 <select  wire:model.defer="role" class="block appearance-none rounded-md w-full border border-gray-200 text-gray-700 py-3 px-4 pr-8 round leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                                  
                    @foreach ($roles as $rolex)
                        @if ($rolex->id == $role)
                          <option value="{{ $rolex->id }}">{{ $rolex->name }}</option> 
                        @else
                        <option value="{{ $rolex->id }}">{{ $rolex->name }}</option>
                     @endif
        ...
        <div class="col-span-6 sm:col-span-6">
            ...
            <x-textarea type="text" class="mt-1 block w-full" wire:model.defer="address" autofocus></x-textarea>
    ...
    <x-slot name="actions">
        <x-back-btn-link-history class="mr-2" >
            {{ __('Back') }}
        </x-back-btn-link-history>
